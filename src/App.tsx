...
import {
  ...
  MapPin, MessageCircle, ChevronDown, Palette,
  Cloud, Cpu, Layers3, Box, Star, Sun, Moon, Languages, User, type LucideIcon
} from 'lucide-react';
...
// Reusable 3D tilt-on-hover effect for cards and the hero portrait.
// Motion only fires in response to the user's own cursor movement.
function useTilt<T extends HTMLElement>(maxTilt = 8) {
  ...
  const onMouseMove = (e: React.MouseEvent) => {
    ...
    setStyle({
      transform: `perspective(900px) rotateY(${px * maxTilt * 2}deg) rotateX(${-py * maxTilt * 2}deg) scale3d(1.02, 1.02, 1.02)`,
    });
  };
...
// Section label: a hairline + word instead of a tracked-out uppercase eyebrow
...
const SkillCard = ({ icon: Icon, name, color }: SkillCardProps) => {
  ...
  return (
    <div className={`group relative backdrop-blur-xl border rounded-xl sm:rounded-2xl p-2.5 sm:p-4 md:p-5 hover:border-red-500/50 transition-all duration-500 hover:scale-105 hover:shadow-xl hover:shadow-red-500/10 ${
      isDark ? 'bg-white/[0.03] border-white/[0.06]' : 'bg-white/80 border-gray-200 shadow-sm hover:shadow-md'
    }`}>
      ...
      <div className="relative z-10 flex flex-col items-center text-center">
        ...
        <span className={`text-[10px] sm:text-xs md:text-sm font-semibold group-hover:text-white transition-colors truncate w-full ${
...
const ServiceCard = ({ icon: Icon, title, description, features }: ServiceCardProps) => {
  ...
  <div
    ...
    style={tilt.style}
    className={`tilt-surface group relative backdrop-blur-xl border rounded-2xl sm:rounded-3xl p-5 sm:p-8 hover:border-red-500/40 transition-colors duration-500 overflow-hidden ${
    ...
    <div className="relative z-10">
      ...
      <p className="text-xs sm:text-sm mb-4 sm:mb-5 leading-relaxed">{description}</p>
...
const ProjectCard = ({ title, subtitle, description, tags, icon: Icon, gradient, featured }: ProjectCardProps) => {
  ...
  <div
    ...
    onMouseLeave={tilt.onMouseLeave}
    style={tilt.style}
    className={`tilt-surface group relative backdrop-blur-xl border rounded-2xl sm:rounded-3xl overflow-hidden hover:border-red-500/40 transition-colors duration-500 ${
    featured ? 'ring-1 ring-red-500/30' : ''
  } bg-white/[0.03] border-white/[0.06]`}>
    ...
    <div className="p-4 sm:p-6">
      <div className="flex items-start justify-between mb-2 sm:mb-3">
        ...
        <button className="w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-white/10 flex items-center justify-center transition-all duration-300">
          <ArrowRight size={12} className="sm:text-sm" />
        </button>
      </div>
      <p className="text-xs sm:text-sm mb-3 sm:mb-4 leading-relaxed">{description}</p>
      <div className="flex flex-wrap gap-1.5 sm:gap-2">
        {tags.map((tag, i) => (
          <span key={i} className="px-2 py-1 sm:px-3 sm:py-1.5 text-xs bg-white/[0.05] border border-white/[0.08] rounded-full transition-all">
            {tag}
          </span>
        ))}
      </div>
...
const TestimonialCard = ({ name, role, text, avatar }: TestimonialProps) => (
  <div className="bg-white/[0.03] backdrop-blur-xl border border-white/[0.06] rounded-2xl sm:rounded-3xl p-4 sm:p-6 hover:border-white/10 transition-all duration-500">
    ...
    <p className="text-xs sm:text-sm leading-relaxed italic mb-3 sm:mb-4">"{text}"</p>
...
// The contact form posts to a small serverless function (api/contact.js)
// instead of calling the Discord webhook directly, so the webhook secret
// never appears in the browser bundle. Defaults to a same-origin call
// (Vercel deploys api/contact.js automatically); set VITE_CONTACT_API_URL
// if the frontend and the API are hosted separately (e.g. static site on
// GitHub Pages calling a Vercel-hosted function). See README.md.
...
function App() {
  ...
  const heroTilt = useTilt<HTMLDivElement>(10);
  ...
  return (
    <div
      ...
      lang={language}
      className={`min-h-screen ${language === 'ar' ? 'lang-ar' : 'lang-en'} ${theme === 'dark' ? 'bg-black' : 'bg-gray-50'} ${theme === 'dark' ? 'text-white' : 'text-gray-900'} overflow-x-hidden`}
    >
      {/* Background */}
      ...
      <section
        ...
        className="min-h-screen flex flex-col justify-center relative pt-20 sm:pt-24 pb-16 overflow-hidden"
      >
        {theme === 'dark' && <div className="hero-spotlight" />}
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-8 lg:py-16 grid lg:grid-cols-[1.1fr_0.9fr] gap-10 sm:gap-12 lg:gap-16 items-center relative z-10 flex-1 w-full">
          {/* Text Content */}
          <div className="space-y-4 sm:space-y-6 lg:space-y-8 text-left">
            <div className="inline-flex items-center gap-2.5 px-3 sm:px-4 py-1.5 sm:py-2 bg-white/[0.03] border border-white/[0.06] rounded-full text-xs sm:text-sm">
            ...
            <h1 className="text-4xl xs:text-5xl sm:text-6xl lg:text-7xl font-bold leading-[1.05]">
              <span>I am </span>
            ...
            <p className="text-sm sm:text-base lg:text-lg leading-relaxed max-w-lg text-gray-400">
              {t.description}
            </p>
            <div className="flex flex-wrap gap-3 sm:gap-4">
              <a href="#projects" className="group px-5 sm:px-6 lg:px-8 py-2.5 sm:py-3 lg:py-4 bg-gradient-to-r from-red-600 to-red-500 rounded-full font-semibold text-sm sm:text-base hover:shadow-xl hover:shadow-red-500/20 transition-all duration-300 flex items-center gap-2">
                {t.viewWork}
                <ArrowRight size={16} className="sm:text-lg group-hover:translate-x-1 transition-transform" />
              </a>
              <a href="#contact" className="px-5 sm:px-6 lg:px-8 py-2.5 sm:py-3 lg:py-4 border border-white/10 rounded-full font-semibold text-sm sm:text-base hover:bg-white/5 transition-all duration-300">
                {t.contactMe}
              </a>
            </div>
            {/* Social Links */}
          ...
          <div className="flex flex-col items-center gap-6 sm:gap-8">
            {/* Avatar with Profile Image — 3D tilt frame with a rotating ring */}
            <div className="relative w-52 h-52 xs:w-60 xs:h-60 sm:w-72 sm:h-72 md:w-80 md:h-80 [perspective:1200px]">
              <div className="absolute -inset-6 rounded-full opacity-70 animate-[spin_10s_linear_infinite]" style={{
                background: 'conic-gradient(from 0deg, transparent 0%, rgba(217,40,45,0.7) 12%, transparent 28%, transparent 72%, rgba(217,40,45,0.4) 88%, transparent 100%)',
                WebkitMaskImage: 'radial-gradient(closest-side, transparent 82%, black 84%)',
                maskImage: 'radial-gradient(closest-side, transparent 82%, black 84%)',
              }} />
              ...
              <div
                ...
                className="tilt-surface relative w-full h-full rounded-full p-[3px] bg-gradient-to-br from-red-600 to-red-500"
              >
                <div className={`w-full h-full rounded-full ${theme === 'dark' ? 'bg-black' : 'bg-white'} p-1.5`}>
            ...
            <div className="flex gap-2 sm:gap-3 flex-wrap justify-center">
              <div className={`px-3 sm:px-4 py-2 sm:py-3 ${theme === 'dark' ? 'bg-white/[0.03]' : 'bg-white'} backdrop-blur-xl border ${theme === 'dark' ? 'border-white/[0.06]' : 'border-gray-200'} rounded-xl text-center hover:border-white/10 transition-all`}>
                <div className="text-lg sm:text-xl md:text-2xl font-black">18</div>
                <div className="text-[8px] sm:text-[9px] md:text-[10px] uppercase tracking-wider">{t.age}</div>
              </div>
              <div className={`px-3 sm:px-4 py-2 sm:py-3 ${theme === 'dark' ? 'bg-white/[0.03]' : 'bg-white'} backdrop-blur-xl border ${theme === 'dark' ? 'border-white/[0.06]' : 'border-gray-200'} rounded-xl text-center hover:border-white/10 transition-all`}>
                <div className="text-lg sm:text-xl md:text-2xl font-black">5+</div>
                <div className="text-[8px] sm:text-[9px] md:text-[10px] uppercase tracking-wider">{t.yearsExp}</div>
              </div>
              <div className={`px-3 sm:px-4 py-2 sm:py-3 ${theme === 'dark' ? 'bg-white/[0.03]' : 'bg-white'} backdrop-blur-xl border ${theme === 'dark' ? 'border-white/[0.06]' : 'border-gray-200'} rounded-xl text-center hover:border-white/10 transition-all`}>
                <div className="text-lg sm:text-xl md:text-2xl font-black text-red-400 flex items-center justify-center gap-1">
                  <MapPin size={14} className="sm:text-base" />
                </div>
                <div className="text-[8px] sm:text-[9px] md:text-[10px] uppercase tracking-wider">{t.saudiArabia}</div>
              </div>
            </div>
          </div>
        </div>

        {/* Scroll Indicator - Fixed position outside section */}
        <div className="absolute bottom-4 sm:bottom-6 left-1/2 -translate-x-1/2 flex flex-col items-center gap-1 sm:gap-2 animate-bounce z-20 pointer-events-none">
          <span className="text-[9px] sm:text-[10px] xs:text-xs uppercase tracking-widest opacity-60">{t.scrollDown}</span>
          <ChevronDown size={14} className="sm:text-base opacity-60" />
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 relative z-10">
          <div className="grid lg:grid-cols-2 gap-8 sm:gap-16 items-center">
            ...
            <div className="space-y-6 sm:space-y-8 text-center lg:text-left">
              <div>
                <SectionLabel>{t.aboutMe}</SectionLabel>
      ...
      <section id="skills" className="py-12 sm:py-16 lg:py-24 relative">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 relative z-10">
          <div className="text-center mb-8 sm:mb-12">
            <SectionLabel>{t.mySkills}</SectionLabel>
      ...
      <section id="services" className="py-12 sm:py-16 lg:py-24 bg-white/[0.01]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 relative z-10">
          <div className="text-center mb-8 sm:mb-12">
            <SectionLabel>{t.whatIDo}</SectionLabel>
      ...
      <section id="projects" className="py-12 sm:py-16 lg:py-24">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 relative z-10">
          <div className="flex flex-col sm:flex-row sm:items-end justify-between mb-8 sm:mb-12 gap-4">
            <div>
              <SectionLabel>{t.featuredProjects}</SectionLabel>
          ...
          <div className="grid lg:grid-cols-2 gap-8 sm:gap-12 lg:gap-16">
            ...
            <div className="space-y-4 sm:space-y-6">
              ...
              <div className="space-y-3 sm:space-y-4">
                {[
                  ...
                  { icon: MessageCircle, label: 'Discord', value: '18fs' }
                ].map((item, i) => (
                  <div key={i} className={`flex items-center gap-3 sm:gap-4 p-3 sm:p-4 ${theme === 'dark' ? 'bg-white/[0.02] border-white/[0.04]' : 'bg-white border-gray-200'} rounded-xl border hover:border-red-500/30 transition-all`}>
                    <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-lg bg-red-500/10 flex items-center justify-center text-red-400 shrink-0">
                      <item.icon size={16} className="sm:text-lg" />
                    </div>
                    <div className="min-w-0">
                      <div className="text-[10px] sm:text-xs text-gray-500">{item.label}</div>
                      <div className="text-white font-medium text-xs sm:text-sm truncate">{item.value}</div>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <form
              ...
              onSubmit={async (e) => {
                ...
                const success = await sendToDiscord(formData.name, formData.email, formData.message);
            ...
            >
              <div className="relative">
                ...
                <input
                  ...
                  className={`w-full pl-11 pr-4 py-3 sm:py-4 ${theme === 'dark' ? 'bg-white/[0.03] border-white/[0.06]' : 'bg-white border-gray-200'} rounded-xl text-white placeholder:text-gray-600 focus:outline-none focus:border-red-500/50 transition-all text-sm sm:text-base`}
                  required
                />
              </div>
              <div className="relative">
              ...
              <div className="relative">
                <MessageCircle size={16} className="absolute left-4 top-4 text-gray-500" />
                <textarea
                  rows={5}
                  placeholder={t.yourMessage}
                  value={formData.message}
                  onChange={(e) => setFormData({ ...formData, message: e.target.value })}
                  className={`w-full pl-11 pr-4 py-3 sm:py-4 ${theme === 'dark' ? 'bg-white/[0.03] border-white/[0.06]' : 'bg-white border-gray-200'} rounded-xl text-white placeholder:text-gray-600 focus:outline-none focus:border-red-500/50 transition-all resize-none text-sm sm:text-base`}
                  required
                />
              </div>
              <button
                ...
                className={`w-full px-6 sm:px-8 py-3 sm:py-4 bg-gradient-to-r from-red-600 to-red-500 rounded-xl font-semibold text-sm sm:text-base hover:shadow-xl hover:shadow-red-500/30 transition-all duration-300 flex items-center justify-center gap-2 ${isSubmitting ? 'opacity-70 cursor-not-allowed' : ''}`}
              >
                {isSubmitting ? (
                  <>
